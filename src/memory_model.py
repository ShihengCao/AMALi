...
def get_hit_rate_analytical(reuse_profile, cache_size, line_size, associativity):
    ...
    for items in reuse_profile:
        ...
        items = items.split(",")
        if len(items) < 2:
            print("Warning! get_hit_rate_analytical(), reuse_profile items only has stack_distance but probability is needed!")
            continue
        # print(items)
        ...
        try:
            ...
            print("Warning! get_hit_rate_analytical(), probability is '' but a number is needed, set as 0")
            probability = 0
        ## compute probability of a hit given D
        if stack_distance == -1:   phit += 0
        elif stack_distance == 0:  phit += probability
        else:
            # print(associativity, num_blocks)
...
def private_SM_computation(SM_id, kernel_id, grid_size, num_SMs, mem_trace_dir_path,\
...
def get_memory_perf(kernel_id, mem_trace_dir_path, grid_size, num_SMs, l1_cache_size, l1_cache_line_size, l1_cache_associativity,\
    ...
    memory_stats["umem_hit_rate"] = sum(umem_hit_rates_list) / max(1,len(umem_hit_rates_list))
    memory_stats["gmem_hit_rate_lds"] = sum(gmem_hit_rates_lds_list) / max(1,len(gmem_hit_rates_lds_list))

    ## ---- atomic & reduction instructions ---- ##
    memory_stats["atom_tot_reqs"] = int(sum(atom_reqs_list) / max(1,len(atom_reqs_list)))
    memory_stats["red_tot_reqs"] = int(sum(red_reqs_list) / max(1,len(red_reqs_list)))
    memory_stats["atom_red_tot_trans"] = int(sum(atom_red_trans_list) / max(1,len(atom_red_trans_list)))

    ## ---- global memory ---- ##
    if memory_stats["lmem_used"]:
        memory_stats["gmem_hit_rate"] = sum(gmem_hit_rates_list) / max(1,len(gmem_hit_rates_list))
    else:
        memory_stats["gmem_hit_rate"] = memory_stats["umem_hit_rate"]
    memory_stats["gmem_ld_reqs"] = int(sum(gmem_ld_reqs_list) )
    memory_stats["gmem_st_reqs"] = int(sum(gmem_st_reqs_list) )
    memory_stats["gmem_tot_reqs"] = memory_stats["gmem_ld_reqs"] + memory_stats["gmem_st_reqs"]
    memory_stats["gmem_ld_trans"] = int(sum(gmem_ld_trans_list) )
    gmem_sm_ld_trans = int(sum(gmem_ld_trans_list) / max(1,len(gmem_ld_trans_list)))
    memory_stats["gmem_sm_ld_trans"] = int(gmem_sm_ld_trans)
    memory_stats["gmem_st_trans"] = int(sum(gmem_st_trans_list) )
    gmem_sm_st_trans = int(sum(gmem_st_trans_list) / max(1,len(gmem_st_trans_list)))
    memory_stats["gmem_sm_st_trans"] = int(gmem_sm_st_trans)
    ...
    ## ---- local memory ---- ##
    if memory_stats["lmem_used"]:
        memory_stats["lmem_hit_rate"] = sum(lmem_hit_rates_list) / max(1,len(lmem_hit_rates_list))
